...
class Kitchen:
    ...
    def setup_task(self):
        ...
        if 'push' in self.task or 'slide' in self.task:
            ...
            if self.task == 'push_kettle_burner':  # single goal test, push to back burner
                ...
                self.goal[-1] = np.squeeze(init_xpos[XPOS_INDICES['kettle']])[
                    -1]  # set z pos to be same as kettle, since we only want to push in x,y
            ...
            if self.task == 'pick_kettle_burner':  # single goal test, slide to back burner
                ...
                self.goal = np.random.uniform(low=[-1, 0, 0], high=[0, 1,
                                                                    0])  # randomly select goal location in workspace OUTSIDE of end effector reach
                self.goal[-1] = np.squeeze(init_xpos[XPOS_INDICES['kettle']])[
                    -1]  # set z pos to be same as kettle, since we only want to slide in x,y
    ...
    def get_reward(self):
        ...
        if 'reach' in self.task:
            ...
            microwave_pos_bottom = self._env.sim.data.site_xpos[
                self._env.sim.model._site_name2id['microwave_door_bottom']]
            ...
            # clip reward so if the block falls off the table we don't get a huge reward
    ...
    def set_xyz_action(self, action):
        ...
        new_mocap_pos = self._env.data.mocap_pos + pos_delta[
            None]  # self._env.sim.data.site_xpos[self.end_effector_index].copy() + pos_delta[None]
    ...
    def single_step(self, action):
        ...
        if self.control_version == 'mocap_ik':
            ...
            ikresult = qpos_from_site_pose(physics, self.end_effector_name, target_pos=xyz_pos, joint_names=joint_names,
                                           tol=1e-10, progress_thresh=10, max_steps=50)
            qpos = ikresult.qpos
    ...
    def get_state(self):
        ...
        if self.state_type == 'all':
            if 'rope' in self.task:
                ...
                state_list.append(
                    np.squeeze(self._env.sim.data.site_xpos[self._env.sim.model._site_name2id['microwave_door']]))
            elif 'open_cabinet' in self.task:
                state_list.append(
                    np.squeeze(self._env.sim.data.site_xpos[self._env.sim.model._site_name2id['cabinet_door']]))
    ...
    def observation_space(self):
        ...
        if not self.state_type == "none":
            ...
            if self.state_type == 'all':
                state_shape += 3
            state_space = gym.spaces.Box(np.array([-float('inf')] * state_shape),
