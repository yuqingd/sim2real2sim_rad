...
def random_translate(imgs, translate_max=4):
    """
        args:
        imgs: np.array shape (B,C,H,W)
        out: output size (e.g. 84)
    ...
    """
    n, c, h, w = imgs.shape
    translate_w1 = np.random.randint(-translate_max, translate_max, n)
    translate_h1 = np.random.randint(-translate_max, translate_max, n)
    translated = np.empty((n, c, h, w), dtype=imgs.dtype)
    for i, (img, w11, h11) in enumerate(zip(imgs, translate_w1, translate_h1)):
        translated[i, :, max(0, h11):h11 + h, max(0, w11):w11 + w] = img[:, max(0, h11):h11 + h, max(0, w11):w11 + w]
    return translated
